...
- **Supabase** — Project Settings → API Keys in your Supabase dashboard. Use the new **Secret key** (`sb_secret_...`) for `SUPABASE_SECRET_KEY`, not the legacy `service_role` JWT — same permissions, but it rotates independently if it ever leaks. The **Publishable key** (`sb_publishable_...`) is optional for now; nothing needs it yet. Free Supabase projects **auto-pause after 1 week of inactivity** — if the connector fails with a DNS-resolution error (`getaddrinfo failed`), check the project isn't paused before assuming the code is broken; after restoring a paused project, also flush your local DNS cache (`ipconfig /flushdns` on Windows) if it still fails right after restore.
