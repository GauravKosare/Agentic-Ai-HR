...
**Version:** 2.2 — STT moved from self-hosted Whisper to Groq-hosted Whisper (multi-model chain); model chains for Gemini/Groq updated from live account data
**Date:** July 20, 2026
...
| **Groq API (STT)** 🔁 | Speech-to-text — replaces self-hosted Whisper (see §4a decision record) | Meeting Bot / Live Interview Agent, `app/connectors/speech_to_text.py` | Real-time audio, every interview minute | Chain: `whisper-large-v3-turbo` → `whisper-large-v3`, each ~20 RPM/2,000 RPD/28.8K audio-seconds-per-day free, verified live 2026-07-20. Same `GROQ_API_KEY` as the LLM fallback above — **shares Groq as a provider**, so a Groq-wide outage hits both at once (correlated, not independent) | Raises `TranscriptionUnavailable` if both models are exhausted — no second STT provider to fall back to yet | ✅ (rate-limited, not cost-limited; see correlated-outage risk in §5) |
...
| **AI4Bharat Indic Parler-TTS / IndicF5** 🔁 | TTS (Hindi/Marathi/English) | Meeting Bot / Live Interview Agent | Real-time audio, every interview minute | Open source, self-hosted, free | N/A — runs on the Oracle Cloud VM above; not yet latency-benchmarked (§4) | ✅ (quality trade-off, see §4) |
| **Claude (Anthropic API)** 🔁 removed | — | — | — | No ongoing free tier | Replaced entirely by the Gemini→Groq LLM Router | — |
| **Sarvam AI** 🔁 removed | — | — | — | No ongoing free tier | Replaced by Groq-hosted Whisper (STT) + self-hosted AI4Bharat (TTS) | — |
| ~~Self-hosted Whisper~~ 🔁 removed | — | — | — | Ran on the Oracle VM, CPU-only, documented latency risk | Replaced by Groq-hosted Whisper (LPU-accelerated, free, resolves the latency risk directly) — see §4a | — |
...
- **Groq correlated-outage risk (new, from §4a):** Groq now serves both the LLM Router's fallback and STT. A Groq-wide outage or account-level issue affects both at once, in the same interview, rather than being two independent failure modes. There's no second STT provider configured as a further fallback yet — if this proves to be a real operational problem in Phase 7 (pilot), it's worth revisiting, e.g. a self-hosted Whisper as a last-resort STT fallback specifically for when Groq itself is down (different from a quota-exhaustion pause).
- **Self-hosting/ops burden:** Vexa and AI4Bharat still need deployment, monitoring, and update effort a managed SaaS would otherwise absorb. This shows up as engineering time (Implementation Plan §2), not a bill, but it's real and should be tracked. (Whisper no longer needs this, per §4a.)
