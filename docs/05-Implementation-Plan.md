...
- **First and highest-priority item this phase:** prototype the **custom Playwright/Chromium meeting bot** (replaces Vexa, TRD §3.7) on the **Oracle Cloud Always Free VM** — joining Zoom/Meet via their browser web-join URLs, plus the **OS-level virtual audio routing** (e.g., a Linux `pulseaudio`/`pipewire` loopback) needed to connect the meeting's audio to the browser tab's `SpeechRecognition` input and route `speechSynthesis` output back out as the bot's mic. This is unbuilt and unproven — validate it works at all before building anything else in this phase on top of the assumption that it does.
- Wire up Chrome's built-in `SpeechRecognition` (STT) and `speechSynthesis` (TTS) as the primary in-browser path, with the already-built **Groq-hosted Whisper chain** and **self-hosted AI4Bharat Indic Parler-TTS/IndicF5** (deployed on the same VM) as fallbacks if either is unavailable or fails mid-call.
...
- **New vs. the original plan:** dedicate explicit test time to (a) quantifying the code-switching accuracy gap between Chrome's `SpeechRecognition` and the originally-specified commercial option (TRD §3.7), and (b) validating Chrome `speechSynthesis` voice availability/quality for Hindi/Marathi on the actual headless Linux VM — a real open risk, not yet tested, that determines whether TTS should stay on Chrome or fall back to AI4Bharat as primary. This is why this phase is 8 weeks instead of 6. Also measure real STT→LLM→TTS latency end-to-end on the actual VM (per TRD §7) rather than assuming the ~1s target is met.
...
| Voice/Speech + Self-Hosting Engineer | Custom Playwright/Chromium bot build (replaces Vexa), virtual audio routing (the highest-uncertainty build item in the project), Chrome Web Speech API integration, AI4Bharat/Groq fallback wiring, and code-switching/voice-quality validation — this role now owns more custom engineering than either the original Sarvam-API-integration scope or the Vexa-based self-hosted plan it replaced |
...
| Meeting Bot + STT/TTS fallback (custom Playwright/Chromium bot + AI4Bharat, on Oracle Cloud Always Free VM) ✅ | $0 — a permanent free ARM VM (currently 2 OCPU/12GB RAM), not a trial; the right architectural fit since this is a long-running stateful process, not a serverless one (TRD §3.7, §4) |
| STT/TTS primary (Chrome `SpeechRecognition`/`speechSynthesis`, in-browser) ✅ | $0, no metering at all — built into Chromium; Groq Whisper is now only a fallback, so the Groq/LLM-Router correlated-outage risk is reduced, not eliminated (tracked in [07-Financial-Subscription-Tracking.md §5](./07-Financial-Subscription-Tracking.md)) |
...
| Self-hosting/custom-build engineering time | Not a subscription cost, but real, and larger than earlier estimates: the custom Playwright bot (replacing Vexa) and its virtual audio routing are now a from-scratch build, plus AI4Bharat's deployment/monitoring — factor generously into the Voice/Speech + Self-Hosting Engineer role's time budget |
