...
| Multilingual accuracy errors (especially Hindi/Marathi code-switching) mislead scoring — **elevated risk**: Chrome's in-browser `SpeechRecognition` (STT) and `speechSynthesis` (TTS), chosen for zero cost, are unbenchmarked for this project and expected to be weaker on code-switching than the purpose-built commercial option originally specified | Dedicated multilingual/code-switching test pass before any real candidate use (TRD §3.7); include human review of transcript alongside score, not score alone; fall back to English-only or human-conducted interview if accuracy is unacceptable for a session |
| Chrome `speechSynthesis` voice quality/availability for Hindi/Marathi on the headless Linux meeting-bot VM is unvalidated — **new, elevated risk** | Test explicitly in Phase 5 before relying on it; self-hosted AI4Bharat TTS is the documented fallback if voice quality proves unacceptable (TRD §3.7, [07-Financial-Subscription-Tracking.md §4b](./07-Financial-Subscription-Tracking.md)) |
| Legal risk from fully automated rejection | Human-in-loop gate before any final adverse action |
| Meeting bot (custom-built, replacing off-the-shelf Vexa) fails to join, or the virtual-audio-routing build proves harder than expected | Auto-retry join; if failure persists, auto-reschedule with apology notification; log incident. Virtual audio routing is flagged as the single highest-uncertainty engineering item in the project — prototype it first in Phase 5, before other interview-pipeline work (TRD §3.7, Implementation Plan Phase 5) |
