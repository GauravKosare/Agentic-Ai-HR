...
Build a single-operator agentic AI system where a recruiter (the "Owner") creates one recruitment platform account, grants the AI Agent access, and then simply tells it *what* to hire for (role, quantity, location type, seniority, must-have skills, etc.). From that point, the AI Agent autonomously:

1. Drafts and floats job/internship forms to connected platforms.
2. Monitors incoming responses continuously.
3. Once a minimum response threshold is met, contacts candidates via Email and WhatsApp Business with interview logistics.
...
5. **Multi-Channel Interview Invitation** — Sends interview date, time, format, time limit, and interview etiquette/protocol via Email and WhatsApp Business, in the candidate's selected language.
6. **Automated Meeting Creation** — At the scheduled time (set by the Owner per requisition or per candidate), the AI creates a Zoom (primary) or Google Meet (secondary) instant meeting and sends the join link via Email/WhatsApp.
...
| FR-6 | System shall send interview invitations via Email and WhatsApp Business containing date, time, format, time limit, and etiquette/protocol instructions | P0 |
| FR-7 | System shall detect/accept candidate's language preference (English, Hindi, Marathi) and communicate accordingly | P0 |
| FR-8 | System shall auto-create a Zoom (primary) or Google Meet (fallback) instant meeting at the scheduled interview time | P0 |
| FR-9 | System shall send the meeting join link via Email and WhatsApp | P0 |
...
| FR-13 | System shall log every automated action (post, message sent, meeting created, score generated) for audit | P0 |
...
- **Compliance:**
  ...
  - **WhatsApp messaging:** Must use Meta's approved message templates and respect opt-in/opt-out; the old WhatsApp on-premise API was deprecated 23 October 2025 — Cloud API is the only supported path.
...
- **Latency:** Real-time interview STT/TTS round-trip should stay under ~1 second to feel conversational.
...
- **Security:** PII encrypted at rest/in transit; role-based access; recordings stored securely with defined retention.
...
| Multilingual accuracy errors (especially Hindi/Marathi code-switching) mislead scoring | Use a model built for Indian-language code-switching (see TRD); include human review of transcript alongside score, not score alone |
...
| WhatsApp opt-out/consent violations | Explicit opt-in captured at application; template messages only; easy opt-out honored immediately |
